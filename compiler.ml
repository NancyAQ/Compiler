 ...
 module Tag_Parser : TAG_PARSER = struct
   ...
   let list_contains_unquote_splicing =
     ormap (function
         ...
         | _ -> false);;
   let names_and_values_of_ribs=
     let rec run=function
      |ScmNil->([],[])
      |ScmPair (ScmPair(name,ScmPair(value,ScmNil)),ribs)->
       let (names,values)=run ribs in
        (name::names,value::values)
      |_->raise(X_syntax "invalid rib structure") 
      in run ;;
   let pairs_of_list s e=
    List.fold_right
    (fun car cdr->ScmPair(car,cdr)) s e;;
 
   let rec macro_expand_let=function
    |ScmPair(ScmSymbol "let",ScmPair(ribs,expr))->
      let(names,values)=names_and_values_of_ribs ribs in
      let params=scheme_sexpr_list_of_sexpr_list names in
      let args=scheme_sexpr_list_of_sexpr_list values in
      ScmPair(ScmPair (ScmSymbol "lambda", ScmPair (params,expr)),args)
     |e->e;;
   let rec macro_expand_let_star=function
     |ScmPair(ScmSymbol "let*",ScmPair(ScmNil,expr))->
       ScmPair(ScmSymbol "let",ScmPair(ScmNil,expr))
     |ScmPair(ScmSymbol "let*",ScmPair(ScmPair(ScmPair(base_d,ScmPair(base_v,ScmNil)),
     ScmNil),expr))->
       ScmPair(ScmSymbol "let",ScmPair(ScmPair(ScmPair(base_d,ScmPair(base_v,ScmNil)),
     ScmNil),expr))
     |ScmPair(ScmSymbol "let*",ScmPair(ScmPair(ScmPair(base_d,ScmPair(base_v,ScmNil)),
     rest),expr))->
       ScmPair(ScmSymbol "let",ScmPair(ScmPair(ScmPair(base_d,ScmPair(base_v,ScmNil)),
     ScmNil),ScmPair(ScmPair(ScmSymbol "let*",ScmPair(rest,expr)),ScmNil)))
     |e->e;;
   let rec macro_Expand_letrec= function
    |ScmPair(ScmSymbol "letrec",ScmPair(ribs,exprs))->
     let(names,values)=names_and_values_of_ribs ribs in
     let dummy_bindings=List.map
       (fun v->
       ScmPair(v,
               ScmPair
                (ScmPair(ScmSymbol "quote",
     ...
     let dummy_ribs=pairs_of_list dummy_bindings ScmNil in
     let sets=
         List.map2
         (fun v e->ScmPair(ScmSymbol "set!",ScmPair(v,ScmPair(e,ScmNil))))
         names values in
     let new_exprs=pairs_of_list sets exprs in
     ScmPair(ScmSymbol "let",ScmPair(dummy_ribs,new_exprs))
    |e->e;;
   ...
   let rec tag_parse sexpr =
     ...
     |ScmPair(ScmSymbol "begin",sequences)->
        (match (scheme_list_to_ocaml sequences) with
         | (sequences', ScmNil) -> ScmSeq (List.map tag_parse sequences')
     ...
     |ScmPair(ScmSymbol "define",ScmPair(ScmPair(v,vals),def))->
       tag_parse(ScmPair(ScmSymbol "define",ScmPair(v,ScmPair(ScmPair(ScmSymbol"lambda",
       ScmPair(vals,def)),ScmNil))))
     |ScmPair(ScmSymbol "define",ScmPair(ScmSymbol v,ScmPair(def,ScmNil)))->
       (* fixed for project *)
       if (is_reserved_word v)
         then raise (X_syntax "Variable cannot be a reserved word")
       else ScmVarDef(Var v,tag_parse def)
     ...
     (* add support for let *)
         | ScmPair (ScmSymbol "let",_) ->
            tag_parse (macro_expand_let sexpr)
     (* add support for let* *)
         | ScmPair (ScmSymbol "let*",_) ->
             tag_parse (macro_expand_let_star sexpr)
     (* add support for letrec *)
     | ScmPair (ScmSymbol "letrec",_) ->
       tag_parse (macro_Expand_letrec sexpr)
 ...
 module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
   ...
   let annotate_lexical_address =
     let rec run expr params env =
       ...
       (* add support for if *)
       |ScmIf(test,dit,dif)->ScmIf'(run test params env,run dit params env
       ,run dif params env)
       (* add support for sequence *)
       |ScmSeq(sequences)->ScmSeq'(List.map(fun exp-> (run exp params env)) sequences)
       (* add support for or *)
       |ScmOr(options)->ScmOr'(List.map(fun exp-> (run exp params env)) options)
       ...
       |ScmLambda(params',Opt opt,expr)->
         ...
         (* fixed for project *)
         ScmLambda' (params', Opt opt, run expr (params'')(params:: env))
         (* ScmLambda' (params', Opt opt, run expr params'(params':: env)) *)
       | ScmApplic (proc, args) ->
   ...
   let annotate_tail_calls = 
     let rec run in_tail = function
       ...
       (* add support for if *)
       | ScmIf'(test',dit',dif')->ScmIf'(run false test',run in_tail dit',
       run in_tail dif')
        (* add support for sequences *)
       | ScmSeq' (exprs) ->
        (match exprs with
           |[]->ScmSeq' ([])
           | head :: tail ->ScmSeq'(runl in_tail head tail))
       (* add support for or *)
       | ScmOr' (exprs) ->
         (match exprs with
            |[]->ScmOr' ([])
            | head :: tail ->ScmOr'(runl in_tail head tail))
       ...
       |ScmApplic'(proc',args',_)->ScmApplic'(run false proc',
...
module Code_Generation (* : CODE_GENERATION *) = struct
  ...
  let const_repr sexpr loc table = match sexpr with
    ...
    | ScmString str ->
      ...
      ([RTTI "T_string"; Quad len; ASCII str],1+len+word_size)
    | ScmSymbol sym ->
    ...
    | ScmVector s ->
      let vect_len=List.length s in
      let addresses=List.map(fun v_item->ConstPtr(search_constant_address v_item table)) s in
       ((RTTI "T_vector") :: (Quad vect_len) :: addresses,1+word_size*(vect_len+1))
    | ScmPair (car, cdr) ->
       let addr1=search_constant_address car table in
       let addr2=search_constant_address cdr table in
       ([RTTI "T_pair"; ConstPtr addr1;ConstPtr addr2], 1 +2*word_size);;
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
      ...
      | ScmBoxGet' var' ->
         ...
         ^ "\tmov rax, qword [rax]\n"
      | ScmBoxSet' (var', expr') ->
        let sandwhich=(run params env expr') in
         let code=(run params env (ScmVarGet' var')) in
         sandwhich
         ^"\tpush rax\n"
         ^code
         ^"\tpop qword [rax]\n"
      ...
      | ScmLambda' (params', Opt opt, body) ->
              ...
              ^ (Printf.sprintf "\tlea rsi, [rsp + 8*%d]\n"
              ...
              ^ "\tcld\n"
              ^ "\tenter 0, 0\n"
              ^ (run (1+List.length params') (env + 1) body)
              ^ "\tleave\n"
              ^ (Printf.sprintf "\tret AND_KILL_FRAME(%d)\n" (1+List.length params'))
              ^ (Printf.sprintf "%s:\t; new closure is in rax\n" label_end)
        ...
        let args_code =
          String.concat ""
            (List.map
               ...
               (List.rev args)) in
        let proc_code = run params env proc in
        let recycle_done=make_tc_applic_recycle_frame_done () in
        let recycle_loop=make_tc_applic_recycle_frame_loop () in
